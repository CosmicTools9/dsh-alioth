//! InboundBom CRUD 集成测试（#[tokio::test] + connect_test_db()，禁 sqlx::test 宏）
mod common;

use ::common::testing::connect_test_db;
use alioth_service_inbound_order::models::{CreateInboundBomRequest, UpdateInboundBomRequest};
use alioth_service_inbound_order::repositories::inbound_bom_repository::InboundBomRepository;
use common::{setup_test_schema, test_code};
use crud::AliothRepository;

#[tokio::test]
async fn inbound_bom_crud_roundtrip() {
    let pool = connect_test_db().await;
    setup_test_schema(&pool).await.expect("setup failed");
    let repo = InboundBomRepository::new(pool.clone());
    let user_id = 1i64;
    let code = test_code("INBOUND_BOM");

    // create
    let created = repo
        .create(
            CreateInboundBomRequest {
