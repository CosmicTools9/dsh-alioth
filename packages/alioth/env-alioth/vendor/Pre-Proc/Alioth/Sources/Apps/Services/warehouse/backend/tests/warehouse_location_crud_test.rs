//! WarehouseLocation CRUD 集成测试（#[tokio::test] + connect_test_db()，禁 sqlx::test 宏）
mod common;

use ::common::testing::connect_test_db;
use alioth_service_warehouse::models::{
    CreateWarehouseLocationRequest, UpdateWarehouseLocationRequest,
};
use alioth_service_warehouse::repositories::warehouse_location_repository::WarehouseLocationRepository;
use common::{setup_test_schema, test_code};
use crud::AliothRepository;

#[tokio::test]
async fn warehouse_location_crud_roundtrip() {
    let pool = connect_test_db().await;
    setup_test_schema(&pool).await.expect("setup failed");
    let repo = WarehouseLocationRepository::new(pool.clone());
    let user_id = 1i64;
    let code = test_code("WAREHOUSE_LOCATION");
