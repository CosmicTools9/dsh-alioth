//! StockCountStatus CRUD 集成测试（#[tokio::test] + connect_test_db()，禁 sqlx::test 宏）
mod common;

use ::common::testing::connect_test_db;
use alioth_service_stock_count::models::{
    CreateStockCountStatusRequest, UpdateStockCountStatusRequest,
};
use alioth_service_stock_count::repositories::stock_count_status_repository::StockCountStatusRepository;
use common::{setup_test_schema, test_code};
use crud::AliothRepository;

#[tokio::test]
async fn stock_count_status_crud_roundtrip() {
    let pool = connect_test_db().await;
    setup_test_schema(&pool).await.expect("setup failed");
    let repo = StockCountStatusRepository::new(pool.clone());
    let user_id = 1i64;
    let code = test_code("STOCK_COUNT_STATUS");
