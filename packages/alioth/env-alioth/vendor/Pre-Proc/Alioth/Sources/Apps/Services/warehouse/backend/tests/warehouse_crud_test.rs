//! Warehouse CRUD 集成测试（#[tokio::test] + connect_test_db()，禁 sqlx::test 宏）
mod common;

use ::common::testing::connect_test_db;
use alioth_service_warehouse::models::{CreateWarehouseRequest, UpdateWarehouseRequest};
use alioth_service_warehouse::repositories::warehouse_repository::WarehouseRepository;
use common::{setup_test_schema, test_code};
use crud::AliothRepository;

#[tokio::test]
async fn warehouse_crud_roundtrip() {
    let pool = connect_test_db().await;
    setup_test_schema(&pool).await.expect("setup failed");
    let repo = WarehouseRepository::new(pool.clone());
    let user_id = 1i64;
    let code = test_code("WAREHOUSE");
