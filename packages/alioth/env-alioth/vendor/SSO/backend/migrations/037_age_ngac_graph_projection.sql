...
AS $fn$
DECLARE
    lbl_ua integer := ag_catalog._label_id('isahl_auth'::name, 'UserAttribute'::name)::integer;
    lbl_oa integer := ag_catalog._label_id('isahl_auth'::name, 'ObjectAttribute'::name)::integer;
    lbl_user integer := ag_catalog._label_id('isahl_auth'::name, 'User'::name)::integer;
    max_rnk bigint;
BEGIN
...
LANGUAGE sql STABLE
AS $fn$
    SELECT 'user_attribute', 0::bigint, 0::bigint, 0::bigint
     WHERE NOT isahl_auth.age_ngac_graph_ready()
    UNION ALL
