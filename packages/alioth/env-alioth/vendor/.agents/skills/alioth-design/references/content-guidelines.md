...
**Alioth 管理后台模块例外**：`alioth-design` 技能产出的标准 CRUD 后台模块原型，必须遵循 `docs/specs/GATEWAY_DESIGN_TOKENS.md` §1.2 与 `docs/specs/HTML_DESIGN_SPEC.md` §1.2 字体体系，使用 **Inter（Display/Body）+ JetBrains Mono（Mono）+ 系统 CJK 栈**（2026-06-14 升级自 Plus Jakarta Sans + ui-monospace），全部走本地 woff2，不得引用 Google Fonts。

**其他场景（品牌页、落地页、营销物料）**：

- ❌ 避免这些烂大街字体：Inter、Roboto、Arial / Helvetica、纯 system font stack、Fraunces、Space Grotesk
- ✅ 用有特点的 display + body 配对（衬线+无衬线、Mono+Sans、Heavy+Light 等）

字体资源：Google Fonts 冷门选项（Instrument Serif、Cormorant、Bricolage Grotesque、JetBrains Mono）、开源字体站、Adobe Fonts。**不要凭空发明字体名**。
