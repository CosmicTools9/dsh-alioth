...
impl LlmBackend for DeepSeekBackend {
    ...
    /// 流式 + 工具调用（D2.3）：复用 `build_body`（与 complete() 同语义，
    /// 含 tools/tool_choice/参数修正）+ `stream:true`；`delta.tool_calls`
    /// 分片由共享 `spawn_sse_tools_parser` 就地累积进 outcome 槽位。
    ...
    fn complete_stream_tools_meta(
        ...
        let stream = futures_util::stream::once(async move {
            // build_body 借用 req，仅在 .json() 序列化前存活（无跨 await 借用）
