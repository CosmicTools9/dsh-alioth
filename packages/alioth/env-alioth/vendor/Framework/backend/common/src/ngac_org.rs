...
//!
//! org 资源行级 OA heal（cover-org-resources-ngac B-2，M221 收编）：岗位
//! `positions` / 部门 `departments` 行级 object attribute 幂等 ensure
//! （[`heal_position_scope`] / [`heal_department_scope`]，含部门子集 OA 树
//! ancestor 闭包与集合 OA 兜底）单一定义落位本模块——identity-org
//! `ngac_org_ensure` 自 M221 起为薄转发，**禁止第二份实现**（消费同源义务）。
//! 语义同 [`ensure_cognition_uas`]：失败仅 warn、绝不阻断 org 主写。
...
pub async fn resolve_member_user_ids(conn: &mut PgConnection, val: &str, limit: i64) -> Vec<i64> {
    if val.is_empty() {
        return Vec::new();
    }
...
/// 行级 OA upsert（仿 Gateway crud::handler::register_created_resource_ngac：
/// o_name `{rt}-{id}`、首个 policy class、resource_identifier；created_by_id=NULL=
/// 系统 ensure）。返回 OA id；唯一键被软删幽灵行占据等退化场景 → None（放弃本次，
/// 留待 Phase C 清理）。
...
async fn ensure_department_chain(pool: &PgPool, org_id: i64) -> Result<Option<i64>, sqlx::Error> {
    ...
    if collection_oa.is_none() {
        ...
        log::warn!("ngac_org: departments:0 集合 OA 缺失（Gateway ngac_seed 未自愈）——部门子集树暂挂空祖先");
    }
...
async fn ensure_position_scope(pool: &PgPool, position_id: i64) -> Result<(), sqlx::Error> {
    ...
    if ancestors.is_empty() {
        ...
        match collection_oa_id(pool, "positions").await? {
            ...
            None => log::warn!("ngac_org: positions:0 集合 OA 缺失——岗位 {position_id} 暂挂空祖先"),
        }
    }
...
/// 岗位 heal 入口（事务外幂等）：行 OA + 在任分配部门闭包 + 集合兜底。
/// NGAC 未启用/任何失败 → 仅 warn（对齐 [`ensure_cognition_uas`] 容错语义），
/// 绝不阻断 org 主写；可安全重放。
pub async fn heal_position_scope(pool: &PgPool, position_id: i64) {
    ...
    if let Err(e) = ensure_position_scope(pool, position_id).await {
        log::warn!("ngac_org: 岗位 {position_id} 行 OA ensure 失败: {e}");
    }
}
...
pub async fn heal_department_scope(pool: &PgPool, org_id: i64) {
    ...
    if let Err(e) = ensure_department_chain(pool, org_id).await {
        log::warn!("ngac_org: 部门 {org_id} 子集 OA ensure 失败: {e}");
    }
}
