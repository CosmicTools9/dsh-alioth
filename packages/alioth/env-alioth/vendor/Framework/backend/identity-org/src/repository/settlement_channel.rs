// SettlementChannelRepository（split 自 repository.rs 单体，④ 候选）
...
use crate::models::{
    CreateSettlementChannelRequest, SettlementChannel, UpdateSettlementChannelRequest,
};
    ...
    > for SettlementChannelRepository
{
    ...
    async fn create(
        ...
        sqlx::query_as::<_, SettlementChannel>(
            r#"INSERT INTO "isahl"."zc_id_stat-smt-channel" (qk_date, qk_income, qk_outgo, qk_amount, "qk_exchange-rate", created_by_id, dk_scene, dk_factor, dk_function)
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
               RETURNING id, qk_date, qk_income, qk_outgo, qk_amount, "qk_exchange-rate", created_at, updated_at, deleted_at"#,
        )
        .bind(req.qk_date).bind(req.qk_income).bind(req.qk_outgo)
        .bind(req.qk_amount).bind(req.qk_exchange_rate)
        .bind(user_id)
