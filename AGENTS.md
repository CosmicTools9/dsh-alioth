...
- **Harness client patches (2026-09-05, AppCreator session gate)**: `scripts/harness-patches/ui-workspace-pick-gate.patch` — a bare New Session lands on the workspace picker (choose the target app first) instead of silently reconnecting the recent workspace. Applied to the harness checkout + `pnpm run build:lib:client && pnpm run build:web` before deploying; the dsh-alioth web gate script opts the deployment in via localStorage `dsh.uiWorkspace.pickOnNewSession`. Re-apply the patch after every harness sync (CI/Docker clones apply it before building).
- **Upstreaming plan (2026-09-06, `docs/upstream-pr-proposal.md`)**: three proposed harness PRs (configurable new-session pick policy; configurable workspace-list scope filter; browser connection account → session owner) would replace the entire client patch set. Identity binding is already non-invasive: `userForSessionId` falls back to the workspace registry path (`Pre-Proc/U-<name>`) when no binding row exists.
