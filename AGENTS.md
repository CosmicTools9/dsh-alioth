...
- **B/S 交付与用户隔离（auth-alioth 插件）**: the plugin group ships as a browser-accessible B/S deployment — users register/log in through the auth API (`POST /api/auth/register|login|logout`, `GET /api/auth/me`; port 3900; node:http, zero deps; scrypt-hashed passwords, bearer-token sessions in `dsh_alioth_auth` — a schema SEPARATE from the registry so `resetRegistry()` never wipes users). Each user owns an Alioth namespace `U-<username>`; a `tools/pre-execute` guard denies alioth_* calls on foreign namespaces (admins span all; first registered user or `ALIOTH_ADMIN_USERNAME/PASSWORD` boots as admin). `mode: 'enforce'` requires an authenticated session for every namespace-scoped call (deployment override `ALIOTH_AUTH_MODE=enforce` for B/S production); `open` keeps headless deployments working. The HTTP API parses both `application/json` and `application/x-www-form-urlencoded` (the login/register pages submit forms). Single shared workspace: isolation is namespace-level, not filesystem-level.
