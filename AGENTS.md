...
- **Host harness source dependency (2026-09-04; no npm harness releases)**: `@deepseek-ai/*` devDependencies pin `0.1.3-alpha.1` and resolve to the sibling `../deepseek-harness` checkout through `pnpm-workspace.yaml` (same pattern as the harness workspace itself including `../dsh-chess`). The checkout must be built first (`pnpm run build:lib:host` in deepseek-harness; watch for stray no-`package.json` dirs under `packages/*/*` that make tsdown read the root manifest and fail entry resolution). CI/Docker clone the harness at tag `dsh-v0.1.3-alpha.1` and build it before installing this workspace. Without the sibling checkout, `pnpm install` fails (the pinned version is not on npm) — that is intentional: adapt against source, never against an older registry line.
