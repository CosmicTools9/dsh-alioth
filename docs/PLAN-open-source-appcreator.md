...
> `pnpm run sync:framework` 重同步过（体积/文件数已变）。**当前 vendor 集的权威判据是
> `pnpm run sync:framework --check`（逐文件 sha256，对上游工作树）与 AGENTS.md 的 Framework 同步段**——
> 逐文件清单 `vendor/PROVENANCE.json` 已于 2026-09-24 退役（见 `docs/builtin-retirement.md`），本表不再作为现状判据。
